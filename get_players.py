...
urlSearchBase = "https://www.pro-football-reference.com/players/"

df_names = pd.DataFrame(columns={'name','years','link'})


for letter in ascii_lowercase:
    urlSearch = urlSearchBase + letter.upper()
    driver.get(urlSearch)
    players = getElements(driver,By.ID,"all_players",2).find_elements(By.TAG_NAME,'p')

    for player in players:
        
        player_str = player.text
        player_name, player_years = [player_str.split(" (")[0],player_str.split(" (")[1].split(") ")[1]]
