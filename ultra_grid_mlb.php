...
$teams_mlb = ['ARI','ATL','BAL','BOS','CHC','CHW','CIN','CLE','COL','DET','HOU','KCR','LAA','LAD','MIA','MIL','MIN','NYM','NYY','OAK','PHI','PIT','SDP','SFG','SEA','STL','TBR','TEX','TOR','WSN']
...
<script>
    ...
    if (Cookies.get('id') == undefined){
        ...
        Cookies.set('id',Math.floor(Math.random() * 1000000),{expires: 9000});
    }

    function resetCookie(){
        Cookies.set('id',Math.floor(Math.random() * 1000000),{expires: 9000});
        Cookies.set( 'filled_mlb_grid', [''], {expires: 9000} );
        Cookies.set( 'players_mlb_grid', [''], {expires: 9000} );
    }
...
$('.alert').eq(0).removeClass('d-none');
    ...
    // var v = Cookies.get('filled_mlb_grid');
    // var v1 = Cookies.get('players_mlb_grid');
    ...
    // } else{
    //     filled_l = Cookies.get('filled_mlb_grid').split(';');
    //     filled_l.forEach(prefillGrid);
    //     filled_players_l = Cookies.get('players_mlb_grid').split(';');
    //     filled_players_l.forEach(prefillTooltips);

    // }
    // var total = filled_players_l.length - 1;
    // $('#status').text(total.toString().concat(' / 420 Correct'))
        });
    ...
    data: { idx: curr_r.toString().concat(',',curr_c.toString()),
        ...
        success: function (response)
                    {
                        if (response){
                            ...
                            // Cookies.set('filled_mlb_grid',v,{expires:9000});
                            // Cookies.set('players_mlb_grid',v1,{expires:9000});
...
<nav class="navbar navbar-expand navbar-light bg-light" style='padding-bottom:0px';>
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
        ...
        <li class="nav-item d-none d-md-block">
            <button type='button' id='nfl-ultra-tab' class="active nav-link"  onclick='window.open("ultra_grid_nfl","_blank")' aria-selected="false" role='tab'>Ultra NFL Grid</a>
        </li>
        </ul>
    </div>
</nav>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-body row">
            <div class="autocomplete col" style="width:300px;">
                <input list='players_mlb_grid' id="search-input" type="text" autocomplete='off'  name="search-input" placeholder="Find Player..." style = "width:100%;height:50px;">
                <datalist id="players_mlb_grid">
                <?php
                ...
                foreach($all_players_mlb as $player){
                    ...
                    if (strpos($keys_players[$_COOKIE['id']],explode(' (',$player)[0]) == false){
...
echo '<div class="row spacing-top"><span class="offset-4 col-4" id="status">/420 Correct</span></div>';
echo '<div class="row logo-row">';
echo '<div class="spacing-left col-1-33"></div>';
echo '<div class="cell col-1-33 empty"></div>';

foreach ($teams_mlb as $team){
    echo '<div class="cell col-1-33 logo-col-div">';
    echo '<img class="logo" src=logos_mlb/' . $team . '.png' . ' />';
    echo '</div>';
}
echo '</div>';

$ct_r = 0;
foreach ($teams_mlb as $team_r){
    echo '<div class="row">';
    echo '<div class="spacing-left col-1-33"></div>';
    echo '<div class="cell col-1-33 logo-row-div">';
    echo '<img class="logo" src=logos_mlb//' . $team_r . '.png' . ' />';
    echo '</div>';
    $ct_c = 0;
    foreach($teams_mlb as $team_c){
        ...
        else { echo '<div class="cell col-1-33">';}
