...
<script>
    ...
    function resetCookie(){
        Cookies.set('id_mlb_roster',Math.floor(Math.random() * 1000000),{expires: 9000});
    }

    if (Cookies.get('id_mlb_roster') == undefined){
        Cookies.set('id_mlb_roster',Math.floor(Math.random() * 1000000),{expires: 9000});
    }
    ...
    data: { key: Cookies.get('id_mlb_roster'),
            type: 'mlb'
        },
    ...
    team_names = ['ARI','ATL','BAL','BOS','CHC','CHW','CIN','CLE','COL','DET','HOU','KCR','LAA','LAD','MIA','MIL','MIN','NYM','NYY','OAK','PHI','PIT','SDP','SFG','SEA','STL','TBR','TEX','TOR','WSN'];
    ...
    function getResults(){
        ...
        $.ajax({
                ...
                data: {
                    ...
                    type: 'mlb',
                    cookie: Cookies.get('id_mlb_roster')
                },
                success: function(response) {
                    if (response == 'already'){
    ...
    function showResultsBanner(indices){
        ...
        for (const idx of indices){
            if (idx != ''){
                $('#show-results-div').append('<img class="logo-icon" src="logos_mlb/'.concat(team_names[idx],'.png" >'));
            }
...
<head>
    ...
    <div class='field-div row'>
        ...
        <div class=" col-6 field field_v1">
            ...
            <input id="search-input" autocomplete="off" class="field__input" placeholder="e.g. Mariners legend Mike Ford">
        </div>
    </div>
    ...
    <?php
        ...
        foreach ($teams as $team) {
            ...
            echo '<img onclick = showModal(\'' . $ct . '\') class="logo" src=logos_mlb/' . $team . '.png' . ' />';
            echo '</div>';
