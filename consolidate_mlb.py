...
if use_selenium:
    ...
    time.sleep(15)
    ct = 0
    for _,name,link in tqdm.tqdm(df.itertuples(),total=df.shape[0]):
        ct += 1
        driver.get(link)
        try:
        ...
        if 'Date' in tbl.columns:
            tbl = pd.read_html(driver.find_elements(By.CLASS_NAME,"stats_table")[1].get_attribute('outerHTML'))[0]
        try:
        ...
        score = ""
        try:
            hof = driver.find_element(By.ID,"div_hof_other")
            found = False
            for row in hof.find_elements(By.TAG_NAME,'td'):
                if found:
                    score = row.find_elements(By.TAG_NAME,'strong')[0].text
                    break
                if row.text == "Hall of Fame Monitor":
                    found = True
        ...
        if len(img_element) != 0:
            ...
            with open("imgs_mlb/" + img_url.split("/")[-1], 'wb') as handler:
                handler.write(img)

        for team in teams:
            if team in teams_directory:
                teams_directory[team].append([name,"",score,img_url])
            else:
                teams_directory[team] = [[name,"",score,img_url]]
        # if ct % 100 == 0:
        with open("sample_mlb_last.json", "w") as outfile:
